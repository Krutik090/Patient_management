...
class PetientData
{
    ...
    {
        $login = $this->conn->prepare("SELECT * FROM users WHERE email= ?");
        $login->execute([$email]);
        ...
        if ($login->rowCount() > 0) {
            ...
            return "Don't Have Any Account With Email, Register First!!!";
        }
    }
